...
exports.getLastFrameScore = exports.calculateAndPushSum = exports.handleRegularFramesAfterStrike = exports.handleSpareFrames = exports.handleTripleStrikeFrames = exports.isRecordingOver = exports.isPinsNumberValid = exports.getFrameLine = exports.getTotalScoreLine = exports.getFrameScoreLine = void 0;
 ...
 */
function getFrameScoreLine(frames) {
    let frameScoreLine = 'Frame score |';
    for (let i = 0; i < frames.length; i++) {
        if (i > constants_1.MAX_NUMBER_COLUMNS - 1) {
...
exports.getFrameScoreLine = getFrameScoreLine;
/**
 ...
 */
function getTotalScoreLine(frames, totalsScore) {
    ...
    if (currentFrame) {
        ...
        if (frames.length > 2) {
            ...
            handleTripleStrikeFrames(previousPreviousFrame, previousFrame, currentFrame, totalsScore);
        }
        ...
        const isRegularFrame = currentFrame.isDone() &&
            ...
            frames.length < constants_1.MAX_NUMBER_COLUMNS + 1;
        if (isRegularFrame) {
    ...
    for (let i = 0; i < totalsScore.length; i++) {
        totalsScore[i] > constants_1.LARGEST_TWO_DIGIT_NUMBER
            ? (totalScoreLine += ` ${totalsScore[i]} |`)
            : (totalScoreLine += `  ${totalsScore[i]} |`);
    }
    return totalScoreLine;
}
exports.getTotalScoreLine = getTotalScoreLine;
/**
...
function getFrameLine() {
    let frameLine = 'Frame       |';
    for (let i = 0; i < constants_1.MAX_NUMBER_COLUMNS; i++) {
        //Il s'agit ou non d'un nombre à 2 chiffres
        frameLine += i < constants_1.MAX_NUMBER_COLUMNS - 1 ? `  ${i + 1}  |` : `  ${i + 1} |`;
    }
    return frameLine;
}
exports.getFrameLine = getFrameLine;
/**
 ...
 */
function isPinsNumberValid(pins, frames) {
    //Récupérer le roll1 s'il existe
    const roll1CurrentFrame = frames.length > 0 && !frames[frames.length - 1].isDone()
        ? frames[frames.length - 1].roll1
        : null;
    //Check si l'addition du roll1 et du roll2 ne dépassent pas 10
    if (roll1CurrentFrame) {
        return roll1CurrentFrame + pins <= constants_1.MAX_NUMBER_COLUMNS;
    }
    //retourne le nombre si les conditions sont validés et pins n'est pas NaN suite au parseInt
    return !Number.isNaN(pins) && pins >= 0 && pins <= constants_1.MAX_NUMBER_COLUMNS;
}
exports.isPinsNumberValid = isPinsNumberValid;
/**
 ...
 */
function isRecordingOver(frames) {
    //Si la fin est atteinte
    if (frames.length > 9) {
        const currentFrame = frames[frames.length - 1];
        const previousFrame = frames[frames.length - 2];
        //Fin régulière
        const isRegularEnd = frames.length > constants_1.MAX_NUMBER_COLUMNS - 1 &&
            currentFrame.roll1 !== undefined &&
            currentFrame.roll2 !== undefined &&
            !currentFrame.isSpare() &&
            !currentFrame.isStrike();
        //Fin en Spare
        const isSpareEnd = frames.length > constants_1.MAX_NUMBER_COLUMNS && previousFrame.isSpare();
        //Fin en Strike
        const isStrikeEnd = frames.length > constants_1.MAX_NUMBER_COLUMNS &&
...
exports.isRecordingOver = isRecordingOver;
/**
 ...
 * @param totalsScore la liste des totaux de score
 */
function handleTripleStrikeFrames(previousPreviousFrame, previousFrame, currentFrame, totalsScore) {
...
exports.handleTripleStrikeFrames = handleTripleStrikeFrames;
/**
 ...
 * @param totalsScore la liste des totaux de score
 */
function handleSpareFrames(previousFrame, currentFrame, totalsScore) {
...
exports.handleSpareFrames = handleSpareFrames;
/**
 ...
 * @param totalsScore la liste des totaux de score
 */
function handleRegularFramesAfterStrike(previousFrame, currentFrame, totalsScore) {
...
exports.handleRegularFramesAfterStrike = handleRegularFramesAfterStrike;
/**
 * Permet d'ajouter le score au total des scores si la liste totalsScore existe, sinon, la crée
 * @param sum total
 * @param totalsScore la liste des totaux de score
 */
function calculateAndPushSum(sum, totalsScore) {
...
exports.calculateAndPushSum = calculateAndPushSum;
/**
 ...
 */
function getLastFrameScore(framesScoreLine, frames) {
    const currentFrame = frames[frames.length - 1];
    const previousFrame = frames[frames.length - 2];
    //Si il s'agit d'un Spare, retire l'existant pour remplacer par les 3 rolls
    if (previousFrame.isSpare()) {
        framesScoreLine = framesScoreLine.slice(0, constants_1.NUMBER_TO_REMOVE_ON_A_FRAME);
...
exports.getLastFrameScore = getLastFrameScore;
